...
class HRDAHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        no_mask_lr_seg = self.head(lr_inp)
        ...
        if has_crop:
            mask = no_mask_lr_seg.new_zeros([no_mask_lr_seg.shape[0], 1, *no_mask_lr_seg.shape[2:]])
            sc_os = self.os / lr_scale
            slc = self.hr_crop_slice(sc_os)
            mask[:, :, slc[0], slc[1]] = 1
            att = att * mask
        # print_log(f'att {att.shape}', 'mmseg')
        lr_seg = (1 - att) * no_mask_lr_seg
        ...
        return fused_seg, lr_seg, hr_seg, no_mask_lr_seg
    ...
    def losses(self, seg_logit, seg_label, seg_weight=None):
        """Compute losses."""
        fused_seg, lr_seg, hr_seg, no_mask_lr_seg  = seg_logit
        loss = super(HRDAHead, self).losses(fused_seg, seg_label, seg_weight)
        loss['no_mask_lr_seg'] = no_mask_lr_seg
