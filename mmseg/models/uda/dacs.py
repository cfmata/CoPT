...
from mmseg.models.uda.domain_attributes import get_domain_templates
...
def cov2(x, rowvar=False, bias=False, ddof=None, aweights=None):
...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        if self.dgtext:
            self.domain_templates = get_domain_templates(cfg['task'])
            self.cityscapes_classes = ['road', 'sidewalk', 'building', 'wall', 'fence', 'pole', 'traffic light',
                                       'traffic sign', 'vegetation', 'terrain', 'sky', 'person', 'rider', 'car',
                                       'truck', 'bus', 'train', 'motorcycle', 'bicycle']
            self.dg_text = torch.zeros((len(self.cityscapes_classes), 512))
            with torch.no_grad():
                model, _ = clip.load("ViT-B/32")
                model.eval()
                i = 0
                for semantic_class in self.cityscapes_classes:
                    completed_templates = [f.format(semantic_class) for f in
                                           self.domain_templates]
                    # CLIP
                    text = clip.tokenize(completed_templates)
                    text_features = model.encode_text(text.cuda())
                    dg_text = text_features.mean(0).detach()
                    self.dg_text[i] = dg_text
                    i += 1

            # Memory bank stores pixel features updated with EMA
            self.use_membank = cfg['membank']
            if self.use_membank:
                self.membank = torch.zeros((len(self.cityscapes_classes), 512))
                self.decay = 0.5
    ...
    def calc_dgtext_dist(self, gt, feat=None):
        assert self.dgtext
        N, C, H, W = gt.shape
        iw1 = torch.linspace(0, W - 1, feat[-1].shape[-1]).long()
        ih1 = torch.linspace(0, H - 1, feat[-1].shape[-2]).long()
        gt1 = gt[:, :, ih1[:, None], iw1]
        valid_classes = [c.cpu().item() for c in torch.unique(gt1) if c != 255]
        # Get current pixel features
        all_pixel_features = torch.empty((len(valid_classes), 512))
        i = 0
        for semantic_class in valid_classes:
            class_mask1 = gt1 == semantic_class
            mt1 = torch.masked_select(feat[-1], class_mask1)
            mt1 = mt1.reshape((512, -1))
            pixel_feature = torch.mean(mt1, dim=1)
            if self.use_membank:
                # Update pixel feature
                pixel_feature = (1 - self.decay) * self.membank[semantic_class].cuda() + self.decay * pixel_feature
                self.membank[semantic_class] = pixel_feature.detach()
            all_pixel_features[i] = pixel_feature
            i+=1
        # Get covariance of pixel features
        pixel_feature_cov = cov(all_pixel_features)
        # Build text embedding covariance for valid classes
        valid_text_cov = cov(self.dg_text[valid_classes])
        # Cosine similarity between cov matrices
        final_text_loss = torch.nn.CosineEmbeddingLoss()(pixel_feature_cov.flatten().unsqueeze(0), valid_text_cov.flatten().unsqueeze(0), torch.Tensor([1]))
        feat_loss, feat_log = self._parse_losses(
            {'loss_dgtext': final_text_loss})
        feat_log.pop('loss', None)
        return feat_loss, feat_log

    def calc_cluster_loss(self, target_feature, source_feature):
        assert self.cluster
        cluster_inputs = torch.cat([target_feature,
                                    source_feature], dim=0)
        domains = torch.cat([torch.ones(target_feature.shape[0]),
                            torch.zeros(source_feature.shape[0])])
        cluster_loss = self.cluster_loss(cluster_inputs, domains, 0, 'train', 19, 'spatial')[0]
        feat_loss, feat_log = self._parse_losses({'loss_cluster': cluster_loss})
        feat_log.pop('loss', None)
        return feat_loss, feat_log
    ...
    def forward_train(self,
        ...
        #no_mask_lr_seg = clean_losses.pop('decode.no_mask_lr_seg')
        seg_debug['Source'] = self.get_model().debug_output
        clean_loss, clean_log_vars = self._parse_losses(clean_losses)
        log_vars.update(clean_log_vars)

        if self.enable_fdist or self.dgtext or self.cluster:
        ...
        if self.print_grad_magnitude:
            ...
            log_vars['seg_grad'] = grad_mag.cpu()
        ...
        if self.enable_fdist:
            ...
            if self.dgtext or self.cluster:
                feat_loss.backward(retain_graph=self.dgtext)
            else:
                feat_loss.backward()
            if self.print_grad_magnitude:
                params = self.get_model().backbone.parameters()
                ...
                log_vars['fdist_grad'] = grad_mag.cpu()
        del clean_loss
        if self.enable_fdist:
            del feat_loss
        ...
        if self.dgtext:
            ...
            if self.print_grad_magnitude:
                ...
                dgtext_grads = [g2 - g1 for g1, g2 in zip(fd_grads, dgtext_grads)]
                grad_mag = calc_grad_magnitude(dgtext_grads)
                mmcv.print_log(f'DGText Grad.: {grad_mag}', 'mmseg')
                log_vars['dgtext_grad'] = grad_mag.cpu()
